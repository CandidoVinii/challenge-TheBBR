    ...
    <div className="flex-col justify-center items-center mt-6 md:grid md:grid-cols-2 md:gap-2">
      {dataFilter.length > 0 && (
        dataFilter.slice(0, page).map((item) => (
          <div key={item.id} className="ring-1 max-w-sm w-80 mx-auto mt-4 max-h-fit bg-slate-300 rounded-xl shadow-lg md:max-w-2xl">
            <div className="md:flex flex-col items-center justify-center">
              ...
              <div className="md:shrink-0 mt-8 rounded-md flex items-center justify-center bg-contain">
                <img className="w-fit mx-auto mt-4 items-center object-cover ring-1 bg-white rounded-md md:w-full h-40" src={item.images[0].asset.url} alt={item.images[0].alt} />
              </div>
